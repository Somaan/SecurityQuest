...
const Leaderboard = () => {
    ...
    function calculateNormalisedScore(completion) {
        if (completion.earnedPoints !== undefined && completion.totalPossiblePoints !== undefined) {
            const normalisedScore = (completion.earnedPoints / completion.totalPossiblePoints) * 100;
        ...
        if (completion.completion_details) {
            ...
            try{
                ...
                details = typeof completion.completion_details === 'string'
            ...
            if (Array.isArray(details)) {
                ...
                details.forEach(detail => {
                    ...
                    if (detail.possiblePoints !== undefined && detail.earnedPoints !== undefined) {
        ...
        if (completion.correct_answers !== undefined && completion.total_questions !== undefined) {
            const percentage = (completion.correct_answers / completion.total_questions) * 100;
    ...
    function calculateTotalScore(user) {
        ...
        quizCompletions.forEach(completion => {
          ...
          if (correctAnswers !== null && correctAnswers !== undefined) {
            ...
            const quizPoints = Math.round((normalisedScore / 100) * totalQuestions * 10);
            totalScore += quizPoints;

            console.log(`Quiz ${completion.quiz_id} with normalised score: ${normalisedScore}%, ${quizPoints} points`);
        }
    });
