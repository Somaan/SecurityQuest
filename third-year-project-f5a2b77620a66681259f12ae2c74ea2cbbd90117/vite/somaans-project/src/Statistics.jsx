...
const Statistics = () => {
  ...
  function normaliseQuizScores(quiz) {
    if (quiz.earnedPoints !== undefined && quiz.totalPossiblePoints !== undefined) {
      return Math.round((quiz.earnedPoints / quiz.totalPossiblePoints) * 100);
    } else if (quiz.completionDetails && Array.isArray(quiz.completionDetails)) {
      let earnedPoints = 0;
      let totalPoints = 0;
  ...
  const calculateStats = () => {
    ...
    if (quizHistory.length > 0) {
      ...
      stats.averageScore = Math.round(normalisedScores.reduce((a, b) => a + b, 0) / normalisedScores.length);
      stats.highestScore = Math.max(...normalisedScores);
      stats.lowestScore = Math.min(...normalisedScores);
      stats.passRate = Math.round((normalisedScores.filter(score => score >= 70).length / normalisedScores.length) * 100);
  ...
  const calculateDifficultyStats = (difficulty) => {
    ...
    const normalisedScores = matchingQuizzes.map(quiz => normaliseQuizScores(quiz));
    const highestScore = Math.max(...normalisedScores);
    const averageScore = Math.round(normalisedScores.reduce((a, b) => a + b, 0) / attempts);
