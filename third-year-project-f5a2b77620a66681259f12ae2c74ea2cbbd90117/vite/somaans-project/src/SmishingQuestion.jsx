...
const SmishingQuestion = ({ question, onAnswer }) => {
  ...
  const handleOptionSelect = (optionId) => {
    if (submitted) return; // Prevent changes
  ...
  const handleSubmit = () => {
    ...
    // - Incorrect answer: 0 points (no penalty for selection in single-choice)
    const earnedPoints = isCorrect ? 10 : 0;
    const maxPoints = 10; // Each smishing question is worth 10 points

    // Calculate percentage score
    const score = isCorrect ? 100 : 0; // Either 100% or 0% for single-choice questions
      ...
      <div className="sms-display">
        <div className="phone-frame">
          <div className="phone-screen">
            ...
            <div className="sms-timestamp">
              {question.messageContent.timestamp}
            </div>

            <div className="sms-bubble">{question.messageContent.message}</div>
          </div>
        </div>
      </div>
        ...
        {question.options.map((option) => (
          <div
            ...
            className={`smishing-option 
              ...
              ${
                submitted && !option.isCorrect && selectedOption === option.id
                  ? "incorrect"
                  : ""
              }`}
